...
If you choose to Quit please wait for $name to reopen."
qd="Quit & Update"
...
#                    if ([ "$count" -le "$totdefer" ] && [ "$timeSinceDeferral" -ge "$hoursToDeferSecs" ]); then
                      ...
                      if [[ "$count" -ge "totdefer" ]]; then
                        ...
                        #sleep 1
                        if [ $x = "Microsoft Teams Helper" ] || [ $x = "Teams" ] || [ $x = "firefox" ] || [ $x = "AdobeReader" ]; then
                          printlog "Force Quit for $x"
                          pkill $x
                          if [[ $? != 0 ]]; then kill -9 $(pgrep "$x"); fi
#                        elif [ $x = "firefox" ]; then
#                          printlog "Force Quit for firefox"
#                          pkill $x
#                          if [[ $? != 0 ]]; then kill -9 $(pgrep "$x"); fi
#                        fi
                        elif [[ $? != 0 ]]; then
...
skype)
    ...
    appNewVersion=$(curl -is "https://get.skype.com/go/getskype-skypeformac" | grep ocation: | grep -o "Skype-.*dmg" | cut -d "-" -f 2 | cut -d "." -f1-4)
    versionKey="CFBundleVersion"
...
#    downloadURL=https://www.github.com$(curl --silent https://github.com/Tunnelblick/Tunnelblick/releases/ | grep -v beta | grep dmg | head -1 | sed -e 's/<a href="//g' -e 's/".*//' | tr -d ' ');
#    if [ ! -z "${9}" ]; then
#      appNewVersion="${9}"
#    fi
#    appNewVersion="3.8.6beta04 (build 5705)"
...
#    tmpDir=$(dirname "$0")
    tmpDir=$(mktemp -d "/private/tmp/$name")
else
    # create temporary working directory
#    tmpDir=$(mktemp -d )
#    tmpDir=$(mktemp -d "/private/tmp/$name")
    dlDir="/private/tmp/${name}"
#    dlDir="/Library/Application Support/${name}"
    if [[ -d "${dlDir}" ]]; then
...
#    tmpDir=$(mktemp -d "/private/tmp/$name")
pathtotmpDir="/private/tmp/$name"
#tmpDir=$(mkdir -p "/private/tmp/$name")
tmpDir=$(mkdir -p "$pathtotmpDir")
tmpDir="$pathtotmpDir"

#tmpDir=$(mktemp -d "/private/tmp/$name")

#     tmpDir=$(mktemp -d "/Library/Application Support/${name}")
  fi
fi
...
autoload is-at-least
#if is-at-least 11.12.42 "${get_foo_vers}"
if [[ -n $appversion ]]; then
    printlog "App is present"
    echo "appNewVersion=$appNewVersion and appVersion=$appversion"
#Working but if the version has a space it poses a problem
...
#TEMPFILE="${FILENAME}.download"
TEMPFILE="${name}.download"
#curl "$URL" -o "$TEMPFILE"

echo "TEST: App Name $name"
dlDir="/private/tmp/${name}"
#dlDir="/Library/Application\ Support/${name}"
...
#if [[ -d "${dlDir}" ]]; then
    printlog "$archiveName exists, skipping download"
 else
...
#      curlDownload=$(curl -v -fsL --show-error ${curlOptions} "$downloadURL" -o "$archiveName" 2>&1)
      curlDownload=$(curl -v -fsL --show-error ${curlOptions} "$downloadURL" -o "$TEMPFILE" 2>&1)
...
#      if ! curl --location --fail --silent "$downloadURL" -o "$archiveName"; then
#      if ! curl --location --fail --silent "$downloadURL" -o "$TEMPFILE"; then
...
#          message="$name update/installation failed. This will be logged, so IT can follow up."
        ...
        printlog "Download Successfull, restoring original downloaded name"
#        mv "${FILENAME}.download" "$FILENAME"
        mv "$TEMPFILE" "$archiveName"
